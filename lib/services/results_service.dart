...
class StudentResult {
  ...
  factory StudentResult.fromCsv(Map<String, String> row, ExamType type) {
    ...
    // ─── الحالة ─────────────────────────────────────────────────────────
    final rawStatus = _findField(row, [
      'status',
      'statut',
      'resultat',
      'résultat',
  ...
  static String _normalizeStatus(String status, ExamType type) {
    ...
    if (lower.contains('exclu') ||
        lower.contains('مطرود') ||
        lower.contains('expuls')) {
      return 'مطرود';
    }
  ...
  bool get isComplementary =>
      status == 'تكميلي' ||
      status == 'الدورة التكميلية' ||
      status == 'مؤهل للدورة التكميلية';
...
class ResultsService {
  ...
  static Future<List<StudentResult>> _parseCsvAsync(
    ...
    try {
      return await compute(_parseCsvIsolated, {'csv': csv, 'type': type});
    } catch (e) {
      ...
      for (int i = 1; i < lines.length; i++) {
        ...
        try {
          ...
          for (int j = 0; j < headers.length; j++) {
            ...
            final val = j < values.length
                ? values[j].trim().replaceAll('"', '').trim()
                : '';
  ...
  static Future<void> _saveToDiskCache(
    ...
    try {
      ...
      await prefs.setString(_metaPrefix + cacheKey, jsonEncode(meta.toJson()));
